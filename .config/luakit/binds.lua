...
-- Change the user-agent of the current webview
-- If restore == true, then the user-agent is reset to its original
-- value
...
add_binds("normal", {
    ...
    buf("^%$" ,                     function (w, b, m) w:scroll_vert(m.count.."%") end, {count = 50}),
    ...
    buf("^gb$",                     function (w, m) w:back(m.count)    end),
    buf("^gf$",                     function (w, m) w:forward(m.count) end),
...
add_cmds({
 ...
 -- cmd("co[mmand]",            function (w, arg, opts) .. end, opts),
    ...
    cmd("b[ack]",               function (w, a) w:back(tonumber(a) or 1) end),
    ...
    cmd({"useragent",   "ua"},  function (w, a, o) set_ua(w, a, o.bang) end),
